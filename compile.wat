...
export func $main() i32 {
  ...
  if i32.$ERROR_LIST(->)$list_count == 0 { 
    $root_node = $parse()
  }
  if i32.$ERROR_LIST(->)$list_count == 0 {
...
func $process_token($s i32, $line i32, $column i32) {
  ...
  } else if $str_eq($s, "->") { $kind = $TokenType_Arrow 
  ...
  loop { br_if $str_index >= $length
    ...
    ;; Identifier
      ...
      if ($get_chr($dwasm, $str_index + 1) == '(') & 
         ...
         ($get_chr($dwasm, $str_index + 3) == '>') & 
         ($get_chr($dwasm, $str_index + 4) == ')') {
        $str_index += 4
        $column += 4
        $chr = $get_chr($dwasm, $str_index)
        $add_token($TokenType_Arrow, $value_str, $line, $column)
      }
...
func $parse_statement() i32 {
  ...
  if $kind ==  $TokenType_Local {
    ...
    if $next_kind == $TokenType_Arrow {
      $node = $parse_arrow_store()
    } else if $next_kind == $TokenType_LParen {
...
func $is_unary_op($token i32) i32 {
  local $kind i32 = $token(->)$token_kind
  $kind == $TokenType_Minus | $kind == $TokenType_Exclam | $kind == $TokenType_Cnt | $kind == $TokenType_Clz | $kind == $TokenType_Ctz
    | $kind == $TokenType_Abs | $kind == $TokenType_Neg | $kind == $TokenType_Ceil | $kind == $TokenType_Floor
    | $kind == $TokenType_Trunc | $kind == $TokenType_Round | $kind == $TokenType_Sqrt 
}
...
func $parse_func_params() i32 {
  ...
  loop { br_if i32.$CURRENT_TOKEN(->)$token_kind == $TokenType_RParen 
...
func $parse_arrow_load() i32 {
  local $node i32 = $new_node($Node_DotLoad)
  local $BodyList i32 = $new_list()
  $node(->)$node_Nodes = $BodyList
  $list_add($BodyList, $parse_identifier())
  loop { br_if !$CURRENT_TOKEN 
    br_if i32.$CURRENT_TOKEN(->)$token_kind != $TokenType_Arrow
    $eat_token($TokenType_Arrow)
    if $is_native_type($CURRENT_TOKEN) {
      i32.$node(->)$node_dataType = $CURRENT_TOKEN(->)$token_kind
      $next_token()
      br
    } else {
      $list_add($BodyList, $parse_identifier())
...
func $emit_type($node i32, $funcNo i32) {
  ...
  local $TypeString i32 = $new_empty_string(1 + $uleb_length($params) + $params + $uleb_length($returns) + $returns)
...
;; A B + i32.load() C + i32.load() D + ???.load()
func $emit_dot_load($node i32) {
  local $data_type i32 = $node(->)$node_dataType
...
func $emit_call($node i32) {
  ...
  } else if $str_eq($name, "f64_promote_f32") {
    $emit_call_args($node, $TokenType_F32)
...
func $infer_call_data_type($node i32) i32 {
  ...
  } else if $str_eq($name, "f32_convert_s_i64") { return $TokenType_F32
  } else if $str_eq($name, "f32_convert_u_i32") { return $TokenType_F32
  } else if $str_eq($name, "f32_convert_u_i64") { return $TokenType_F32
  } else if $str_eq($name, "f32_reinterpret_i32") { return $TokenType_F32
  } else if $str_eq($name, "f64_promote_f32") { return $TokenType_F64
  } else if $str_eq($name, "f64_convert_s_i32") { return $TokenType_F64
  } else if $str_eq($name, "f64_convert_s_i64") { return $TokenType_F64
  } else if $str_eq($name, "f64_convert_u_i32") { return $TokenType_F64
  } else if $str_eq($name, "f64_convert_u_i64") { return $TokenType_F64
  } else if $str_eq($name, "f64_reinterpret_i64") { return $TokenType_F64
  } else {
    local $resolved_node i32 = $scope_resolve($CURRENT_SCOPE, $name, $node(->)$node_Token)
...
func $str_to_i32($string i32, $token i32) i32 {
  return i32_wrap_i64($str_to_i64($string, $token))
}
...
func $str_to_f32($string i32) f32 {
  return f32_demote_f64($str_to_f64($string))
}
...
func $str_to_f64($string i32) f64 {
  ...
  loop { br_if $offset >= $length 
    ...
    if $chr == '.' {
      ...
      if $isAfterDot { 
        $f += f64_convert_s_i32($chr - '0') / $d
        $d = $d * 10
      } else {
        if $chr >= '0' & $chr <= '9' {
          $f = $f * 10 + f64_convert_s_i32($chr - '0')
        }
      }
    }
    $offset += 1
...
func $append_str($string i32, $append i32) {
  ...
  local mut $offset i32 = 0
  loop { br_if $offset >= $append_length 
...
func $get_chr($string i32, $offset i32) i32 {
  return i32_load8_u($string(->)$string_bytes + $offset)
}
...
global $TokenType_LParen        i32 = 10  ;; Symbols
...
;; Pierre Rossouw 2019-07-21 https://github.com/PierreRossouw/
