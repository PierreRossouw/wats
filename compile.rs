...
// A WebAssembly language with self-hosted compiler.
// The syntax is rust-like but the language is very primitive - definitely not rust.
...
enum Error {
  ...
  emit_node, InvalidOperator, NotMutable
}
...
fn add_token(kind: i32, Value: i32, line: i32, column: i32) {
...
fn process_token(Val: i32, line: i32, column: i32) {
  let mut kind: i32 = Token::Identifier;
  if str_eq_char(Val, '(') { kind = Token::lParen;  
  } else if str_eq_char(Val, ',') { kind = Token::comma; 
  } else if str_eq_char(Val, ')') { kind = Token::rParen; 
  } else if str_eq_char(Val, '{') { kind = Token::lBrace; 
  } else if str_eq_char(Val, '}') { kind = Token::rBrace; 
  } else if str_eq_char(Val, ':') { kind = Token::colon; 
  } else if str_eq_char(Val, ';') { kind = Token::semicolon; 
  } else if str_eq_char(Val, '=') { kind = Token::Assign; 
  } else if str_eq_char(Val, '<') { kind = Token::Lt;
  } else if str_eq_char(Val, '>') { kind = Token::Gt;
  } else if str_eq_char(Val, '+') { kind = Token::Add;
  } else if str_eq_char(Val, '-') { kind = Token::Sub;
  } else if str_eq_char(Val, '*') { kind = Token::Mul; 
  } else if str_eq_char(Val, '/') { kind = Token::Div; 
  } else if str_eq_char(Val, '!') { kind = Token::Not;
  } else if str_eq_char(Val, '%') { kind = Token::Rem;
  } else if str_eq_char(Val, '^') { kind = Token::Xor;
  } else if str_eq_char(Val, '&') { kind = Token::And; 
  } else if str_eq_char(Val, '|') { kind = Token::Or; 
  } else if str_eq_char(Val, '<<') { kind = Token::Shl; 
  } else if str_eq_char(Val, '>>') { kind = Token::Shr;
  } else if str_eq_char(Val, '::') { kind = Token::doubleColon; 
  } else if str_eq_char(Val, '&&') { kind = Token::BoolAnd; 
  } else if str_eq_char(Val, '||') { kind = Token::BoolOr; 
  } else if str_eq_char(Val, '->') { kind = Token::arrow; 
  } else if str_eq_char(Val, '==') { kind = Token::Eql; 
  } else if str_eq_char(Val, '!=') { kind = Token::Ne;
  } else if str_eq_char(Val, '<=') { kind = Token::Le;
  } else if str_eq_char(Val, '>=') { kind = Token::Ge; 
  } else if str_eq_char(Val, '%+') { kind = Token::Remu;
  } else if str_eq_char(Val, '/+') { kind = Token::Divu; 
  } else if str_eq_char(Val, '>+') { kind = Token::Gtu;
  } else if str_eq_char(Val, '<+') { kind = Token::Ltu;
  } else if str_eq_char(Val, '<=+') { kind = Token::Leu; 
  } else if str_eq_char(Val, '>=+') { kind = Token::Geu;
  } else if str_eq_char(Val, '>>+') { kind = Token::Shru;
  } else if str_eq_char(Val, 'fn') { kind = Token::Fun;
  } else if str_eq_char(Val, 'if') { kind = Token::If; 
  } else if str_eq_char(Val, 'pub') { kind = Token::Pub;
  } else if str_eq_char(Val, 'let') { kind = Token::Let;
  } else if str_eq_char(Val, 'mut') { kind = Token::Mut;
  } else if str_eq_char(Val, 'mod') { kind = Token::Mod;
  } else if str_eq_char(Val, 'loop') { kind = Token::Loop;
  } else if str_eq_char(Val, 'enum') { kind = Token::Enum;
  } else if str_eq_char(Val, 'else') { kind = Token::Else;
  } else if str_eq_char(Val, 'true') { kind = Token::True; 
  } else if str_eq_char(Val, 'false') { kind = Token::False;
  } else if str_eq_char(Val, 'break') { kind = Token::Break;
  } else if str_eq_char(Val, 'const') { kind = Token::Const;
  } else if str_eq_char(Val, 'while') { kind = Token::While;
  } else if str_eq_char(Val, 'static') { kind = Token::Static;
  } else if str_eq_char(Val, 'return') { kind = Token::Return;
  } else if str_eq_char(Val, 'continue') { kind = Token::Continue;
  } else if str_eq_char(Val, 'i32') { kind = Token::I32;
  } else if str_eq_char(Val, 'i64') { kind = Token::I64;
  } else if str_eq_char(Val, 'f32') { kind = Token::F32;
  } else if str_eq_char(Val, 'f64') { kind = Token::F64;
  } else if str_eq_char(Val, 'bool') { kind = Token::Bool; }
  add_token(kind, Val, line, column);
}
...
fn lexx(dwasm: i32) {
  ...
  while pos < length { 
    ...
    // Identifiers & reserved words
      ...
      Val = sub_str(dwasm, start, pos - start + 1);
      process_token(Val, line, column);
      if get_chr(dwasm, pos + 1) == '.' & is_alpha(get_chr(dwasm, pos + 2)) {
        pos = pos + 1;
        column = column + 1;
        chr = get_chr(dwasm, pos);
        add_token(Token::dot, Val, line, column);
      }
    ...
    // Single quoted chars | long chars up to 64 bit
      ...
      Val = sub_str(dwasm, start, pos - start);
      decode_str(Val);
      add_token(Token::CharLiteral, Val, line, column);
    ...
    // Number literals, for example -42, 3.14, 0x8d4f0
      ...
      Val = sub_str(dwasm, start, pos - start + 1);
      add_token(Token::NumLiteral, Val, line, column);
    ...
    // Mathematical operators
      ...
      if is_operator_chr(get_chr(dwasm, pos + 1)) {
        if is_operator_chr(get_chr(dwasm, pos + 2)) {
          Val = sub_str(dwasm, pos, 3);
          pos = pos + 2;
          column = column + 2;
        } else {
          Val = sub_str(dwasm, pos, 2);
          pos = pos + 1;
          column = column + 1;
        }
      } else {
...
fn fn_scope(Scope: i32) -> i32 {
  let mut FunScope: i32 = Scope;
  while FunScope {
    if FunScope.scope_Node.node_kind.i32 == Node::Fun { break; }
    if FunScope.scope_Node.node_kind.i32 == Node::Module { break; }
...
fn scope_register_name(Scope: i32, Name: i32, node: i32, token: i32) {
  ...
  let nodeKind: i32 = node.node_kind;
  list_add_name(Scope.scope_Symbols, node, Name);
  if nodeKind == Node::Variable | nodeKind == Node::Parameter {
    let FunScope: i32 = fn_scope(Scope);
    let nextIndex: i32 = FunScope.scope_localIndex;
...
fn scope_resolve(Scope: i32, Name: i32, token: i32) -> i32 {
  let mut node: i32 = 0;
  let mut RecurseScope: i32 = Scope;
  while RecurseScope {
    node = list_search(RecurseScope.scope_Symbols, Name);
    if node { break; }
...
const node_Scope:      i32 = 16;  // Scope for Module/Block/loop/fn used for name resolution
const node_ANode:      i32 = 20;  // Binary left, Call fn, return Expression, Block, Or fn body
const node_BNode:      i32 = 24;  // Binary/Unary right, else Block, fn return, Variable assignment
const node_CNode:      i32 = 28;  // If statement condition node
const node_Nodes:      i32 = 32;  // List of child Node for Module/Block, enums, Or fn locals
...
fn next_token() {
  ...
  let NextTokenItem: i32 = CurrentTokenItem.item_Next;
  if NextTokenItem {
...
fn parse_const() -> i32 {
  ...
  if CurrentScope.scope_Parent.i32 {
    let FunScope: i32 = fn_scope(CurrentScope);
    let FunNode: i32 = FunScope.scope_Node;
...
fn emit_native_global(node: i32) {
  ...
  let Value: i32 = node.node_BNode.node_String;
  ...
  if nodeType == Token::True {
    ...
    append_f64(WASM, str_to_f64(Value));
  } else if dataType == Token::F32 {
    append_f32(WASM, str_to_f32(Value));
  } else if dataType == Token::I64 {
    append_sleb64(WASM, str_to_i64(Value, node.node_BNode.node_Token));
  } else {
    append_sleb32(WASM, str_to_i32(Value, node.node_BNode.node_Token));
  }
...
fn emit_node(node: i32) {
  ...
  if kind == Node::Block {
    ...
    add_error(Error::emit_node, node.node_Token);
  }
}
...
fn emit_assign(node: i32, isExpression: bool) {
  let ResolvedNode: i32 = scope_resolve(CurrentScope, node.node_ANode.node_String, node.node_Token);
  let dataType: i32 = ResolvedNode.node_type;
  let BNode: i32 = node.node_BNode;
  let assigns: i32 = ResolvedNode.node_assigns;
  if assigns == 0 { 
    add_error(Error::NotMutable, node.node_Token);
  }
  if assigns > 0 {
    ResolvedNode.node_assigns = assigns - 1;
  }
  node.node_dataType = dataType;
  ...
  if ResolvedNode.node_Scope == GlobalScope {
    append_byte(WASM, 0x24);  // set_global
    if isExpression {
      append_uleb(WASM, ResolvedNode.node_index);
...
fn emit_identifier(node: i32) {
  let ResolvedNode: i32 = scope_resolve(CurrentScope, node.node_String, node.node_Token);
  let mut dataType: i32 = ResolvedNode.node_dataType;
...
fn emit_call(node: i32) {
  ...
  if str_eq_char(Name, 'i64_i32') {
    ...
    let ResolvedNode: i32 = scope_resolve(CurrentScope, Name, node.node_Token);
    if ResolvedNode {
      emit_fn_call_args(node, ResolvedNode);
...
fn parse_error_list() {
  ...
  if ErrorItem {
    ...
    while ErrorItem {
      ...
      } else if errorNo == Error::emit_node {
...
// Pierre Rossouw 2017  https://github.com/PierreRossouw/rustywasm
